...
public class ShieldtemInitializer implements ItemTemplateInitializer {
    ...
    public List<Item> initialize(ItemTemplate template, ToolMaterial material, ArmorMaterial armorMaterial, Identifier identifier) {
        return Collections.singletonList(Registry.register(Registry.ITEM, identifier, new TemplatedShield(material,new Item.Settings().maxCount(1).group(ItemGroup.COMBAT))));
    }
    ...
    public boolean isValid(ItemTemplate template, Identifier toolMaterial) {
        ...
        return template.getType().contains("shield")
