...
import hu.frontrider.blockfactory.item.templates.TemplatedPickaxe;
...
public class PickaxeItemInitializer implements ItemTemplateInitializer {
    ...
    public List<Item> initialize(ItemTemplate template, ToolMaterial material, ArmorMaterial armorMaterial, Identifier identifier) {
        return Collections.singletonList(Registry.register(Registry.ITEM, identifier, new TemplatedPickaxe(material,template)));
    }
    ...
    public boolean isValid(ItemTemplate template, Identifier toolMaterial) {
        ...
        return template.getType().contains("pickaxe")
